...
Asciidoctor Reducer uses a collection of Asciidoctor extensions to rebuild the AsciiDoc source as a single document.
...
As it goes, it also removes lines that have been excluded by the preprocessor conditionals as well as the directive lines themselves.

Finally, it loads the document again and returns it.
The reduced source is available on the reconstructed document.
